...
//  FBStickerView
//
//  Created by FBLab on 23/4/27.
//  Copyright © 2023年 FBLab. All rights reserved.
//

#import <UIKit/UIKit.h>


typedef NS_ENUM(NSInteger, FBStickerViewScaleMode) {
    FBStickerViewScaleModeBounds,    //通过改变self.bounds达到缩放效果
    FBStickerViewScaleModeTransform  //通过改变self.transform达到缩放效果
};

typedef NS_ENUM(NSInteger, FBStickerViewCtrlType) {
    FBStickerViewCtrlTypeGesture,    //手势,无控制图，双指控制旋转和缩放
    FBStickerViewCtrlTypeOne,        //一个控制图，同时控制旋转和缩放,
    FBStickerViewCtrlTypeTwo         //两个控制图，一个控制旋转，一个控制缩放
};



@interface FBStickerView : UIView
...
@property (nonatomic) FBStickerViewScaleMode scaleMode;
@property (nonatomic) FBStickerViewCtrlType ctrlType;
