...
- (UILabel *)sliderLabel{
    if (!_sliderLabel) {
        _sliderLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, -FBWidth(11), FBWidth(25), FBHeight(10))];
        [_sliderLabel setTextAlignment:NSTextAlignmentCenter];
        [_sliderLabel setFont:FBFontRegular(12)];
...
- (UIView *)slideBar{
    if (!_slideBar) {
        ...
        _slideBar.layer.cornerRadius = FBHeight(2);
...
- (UIView *)splitPoint{
    if (!_splitPoint) {
        _splitPoint = [[UIView alloc] initWithFrame:CGRectMake(0, 0, FBWidth(2), FBHeight(10))];
...
- (instancetype)initWithFrame:(CGRect)frame{
    ...
    if (self) {
        ...
        [self setBackgroundColor:FBColors(238, 0.5)];
        self.minimumTrackTintColor = [UIColor clearColor];
        self.maximumTrackTintColor = [UIColor clearColor];
        
        [self.sliderLabel setTextColor:FBColors(238, 0.9)];
        self.slideBar.backgroundColor = FBColors(238, 1.0);
        self.splitPoint.backgroundColor = FBColors(238, 0.5);
        // 滑块背景
        [self setThumbImage:[self resizeImage:[UIImage imageNamed:@"slider.png"] toSize:CGSizeMake(FBWidth(15), FBWidth(15))] forState:UIControlStateNormal];
        [self setThumbImage:[self resizeImage:[UIImage imageNamed:@"slider.png"] toSize:CGSizeMake(FBWidth(15), FBWidth(15))] forState:UIControlStateHighlighted];
        self.layer.cornerRadius = FBHeight(2);
...
- (void)refreshWithValue:(float)value isSet:(BOOL)set{
    ...
    self.sliderLabel.center = CGPointMake(self->_trackRect.origin.x + FBWidth(15)/2,-self.center.y+FBHeight(10));
...
- (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event{
    ...
    if ((point.y >= -FBWidth(15)) && (point.y < _trackRect.size.height + FBWidth(15))) {
        float value = 0.0;
...
- (BOOL)pointInside:(CGPoint)point withEvent:(UIEvent *)event{
    ...
    if (!result && point.y > -10) {
        if ((point.x >= _trackRect.origin.x - FBWidth(15)) && (point.x <= (_trackRect.origin.x + _trackRect.size.width + FBWidth(15))) && (point.y < (_trackRect.size.height + FBWidth(15)))) {
            result = YES;
        }
...
- (void)layoutSubviews
{
    ...
    self.splitPoint.frame = CGRectMake(self.frame.size.width/2, -FBHeight(10)/2 + FBHeight(4)/2, FBWidth(2), FBHeight(10));
