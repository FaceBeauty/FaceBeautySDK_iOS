//
//  FBBtMakeupCollectionViewCell.m
//  FaceBeautyDemo
//
//  Created by Eddie on 2023/9/11.
//

#import "FBBtMakeupCollectionViewCell.h"
#import "FBUIConfig.h"
#import "FBTool.h"

@interface FBBtMakeupCollectionViewCell ()
...
- (UIImageView *)downloadIcon{
    if (!_downloadIcon) {
        _downloadIcon = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"fb_download"]];
    }
...
- (UIImageView *)downloadingIcon{
    if (!_downloadingIcon) {
        _downloadingIcon = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"fb_downloading"]];
    }
...
- (UIView *)selectMaskView {
    if (!_selectMaskView) {
        ...
        _selectMaskView.backgroundColor = FBColors(0, 0.4);
        _selectMaskView.layer.cornerRadius = FBWidth(5);
...
- (UIImageView *)lineView{
    if (!_lineView) {
        ...
        _lineView.image = [UIImage imageNamed:@"fb_line"];
    }
    return _lineView;
}
...
//        _title.userInteractionEnabled = NO;
        _title.font = FBFontRegular(12);
        _title.textColor = FBColors(255, 1.0);
    }
    return _title;
}
...
- (void)setNoneImage:(BOOL)selected isThemeWhite:(BOOL)isWhite{
    ...
    self.title.text = [FBTool isCurrentLanguageChinese] ? @"无" : @"None";
    if (selected) {
        self.title.textColor = isWhite ? [UIColor blackColor] : MAIN_COLOR;
    }else{
        self.title.textColor = isWhite ? [UIColor blackColor] : FBColors(255, 1.0);
    }
...
- (void)setModel:(FBModel *)model type:(NSInteger)type isThemeWhite:(BOOL)isWhite{
    ...
    [self.htImageView setImage:[UIImage imageNamed:@"FBImagePlaceholder"]];
    
    NSString *iconUrl = [[FaceBeauty shareInstance] getMakeupUrl:(int)type];
    NSString *folder = model.category;
    NSString *cachePaths = [[FaceBeauty shareInstance] getMakeupPath];
//    NSLog(@"================ %@ ==== %@ ---- %@ ---- %@", iconUrl, cachePaths, folder, [NSString stringWithFormat:@"%@%@",iconUrl, model.icon]);
    [FBTool getImageFromeURL:[NSString stringWithFormat:@"%@%@",iconUrl, model.icon] folder:folder cachePaths:cachePaths downloadComplete:^(UIImage * _Nonnull image) {
