...
@interface FBMattingEffectViewCell ()
...
@implementation FBMattingEffectViewCell
...
- (id)initWithFrame:(CGRect)frame{
    ...
    if (self)
    {
        ...
        [self.htImageView mas_makeConstraints:^(MASConstraintMaker *make) {
            ...
            make.width.height.mas_equalTo(FBWidth(47));
        }];
...
- (void)setDataArray:(NSArray *)dataArray index:(NSInteger)index{
    ...
    if (index == 0) {
        [self setHtImage:[UIImage imageNamed:@"fb_none.png"] isCancelEffect:YES];
        [self setSelectedBorderHidden:YES borderColor:UIColor.clearColor];
    }else{
        
        FBModel *model = [[FBModel alloc] initWithDic:dataArray[index-1]];
        ...
        if([model.category isEqualToString:@"upload_gsseg"]){
            ...
            NSString *iconUrl = [[FaceBeauty shareInstance] getChromaKeyingUrl];
            NSString *folder = model.name;
            NSString *cachePaths = [[FaceBeauty shareInstance] getChromaKeyingPath];
