//
//  FBMakeupSwitchView.m
//  FaceBeautyDemo
//
//  Created by Eddie on 2024/8/17.
//

#import "FBMakeupSwitchView.h"
#import "FBUIConfig.h"
#import "FBTool.h"

@interface FBMakeupSwitchView ()<UICollectionViewDataSource,UICollectionViewDelegate>
...
static NSString *const FBMakeupSwitchViewCellId = @"FBMakeupSwitchViewCellId";

@implementation FBMakeupSwitchView
...
- (UICollectionView *)collectionView{
    if (!_collectionView) {
        ...
        layout.itemSize = CGSizeMake(FBWidth(26), FBHeight(26));
        ...
        [_collectionView registerClass:[FBMakeupSwitchViewCell class] forCellWithReuseIdentifier:FBMakeupSwitchViewCellId];
...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        [self.collectionView mas_makeConstraints:^(MASConstraintMaker *make) {
            ...
            make.width.mas_equalTo(FBWidth(26)*tempArray.count + (tempArray.count-1)*8 + 20);
        }];
    }
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    FBMakeupSwitchViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:FBMakeupSwitchViewCellId forIndexPath:indexPath];
    
    BOOL sel = NO;
    if(indexPath.item == [FBTool getFloatValueForKey:FB_MAKEUP_POSITION_MAP[self.idcard]]){
        sel = YES;
    }
...
//    }
    ...
    // 保存选择
    [FBTool setFloatValue:indexPath.item forKey:FB_MAKEUP_POSITION_MAP[self.idcard]];
        
    // 通知执行美妆效果
    [[NSNotificationCenter defaultCenter] postNotificationName:@"FBMakeupColorSwitchNotification" object:@{@"idCard": @(self.idcard), @"color": colorNameKey}];
...
//    [[FaceBeauty shareInstance] setGSSegEffect:@"" curtainColor:@"#00ff00"];
    ...
    // 通知执行美妆效果
    [[NSNotificationCenter defaultCenter] postNotificationName:@"FBMakeupColorSwitchNotification" object:@{@"idCard": @(self.idcard), @"color": self.dataArray[indexPath.row][@"color"]}];
...
@interface FBMakeupSwitchViewCell ()

@property (nonatomic, strong) UIView *contentV;

@end

@implementation FBMakeupSwitchViewCell
